...
interface SentMessage {
    ...
    title: string;
    body: string;
    recipient_type: 'club' | 'group' | 'user' | 'coaches';
...
interface ManagerProps { role: 'manager'; sent: SentMessage[]; groups: Group[]; athletes: Person[]; coaches: Person[]; }
interface CoachProps   { role: 'coach';   sent: SentMessage[]; inbox: InboxMessage[]; groups: Group[]; athletes: Person[]; }
interface AthleteProps { role: 'athlete'; messages: InboxMessage[]; }
type Props = ManagerProps | CoachProps | AthleteProps;

/* ─── Helpers ─── */

function recipientLabel(msg: SentMessage) {
...
function SentList({ messages, onDelete }: { messages: SentMessage[]; onDelete: (id: number) => void }) {
                        ...
                        <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                    <span className={`px-2 py-0.5 rounded-md text-xs font-bold ${label.cls}`}>{label.text}</span>
                                    {msg.message_type === 'important' && (
                                        <span className="px-2 py-0.5 rounded-md text-xs font-bold bg-red-50 text-red-600 flex items-center gap-1">
    ...
    if (messages.length === 0) {
        return (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-12 text-center">
                ...
                <p className="font-bold text-gray-900 text-sm">No messages from manager yet</p>
            </div>
        );
    }
            ...
            {messages.map(msg => (
                <div
                    ...
                    className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-all ${
                        msg.message_type === 'important'
                            ? (!msg.is_read ? 'border-red-300' : 'border-red-100')
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-2 mb-0.5">
                                    <div className="flex items-center gap-2 min-w-0">
                                        <p className={`font-bold text-gray-900 truncate ${!msg.is_read ? (msg.message_type === 'important' ? 'text-red-900' : 'text-indigo-900') : ''}`}>
                                            {msg.title}
                                        </p>
                                        {msg.message_type === 'important' && (
                                            <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-600 uppercase tracking-wide">Alert</span>
                                        )}
                                    </div>
...
function ComposeModal({ groups, athletes, coaches = [], isManager, onClose }: ComposeModalProps) {
    const form = useForm({
        title:             '',
        body:              '',
        recipient_type:    'club' as 'club' | 'group' | 'user' | 'coaches',
        training_group_id: '',
        recipient_user_id: '',
        message_type:      'regular' as 'regular' | 'important',
    });

    // local state: when recipient_type='user', which list to show
    const [userListTarget, setUserListTarget] = useState<'athlete' | 'coach'>('athlete');

    const IMPORTANT_MAX = 200;
    const bodyMax = form.data.message_type === 'important' ? IMPORTANT_MAX : 5000;
    const bodyRemaining = bodyMax - form.data.body.length;

    const selectRecipient = (type: 'club' | 'group' | 'user' | 'coaches', target?: 'athlete' | 'coach') => {
        form.setData({
            ...form.data,
            recipient_type:    type,
            training_group_id: '',
    ...
    const isActiveRecipient = (type: 'club' | 'group' | 'user' | 'coaches', target?: 'athlete' | 'coach') => {
    ...
    const alertBtnCls = (active: boolean) =>
        `py-2.5 px-3 rounded-xl border-2 text-xs font-bold transition-all ${
            active ? 'border-red-400 bg-red-50 text-red-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'
        }`;

    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
                <div className={`px-6 py-5 ${form.data.message_type === 'important' ? 'bg-gradient-to-r from-red-600 to-orange-600' : 'bg-gradient-to-r from-indigo-600 to-blue-700'}`}>
                    <h3 className="text-lg font-bold text-white">New Message</h3>
                    <p className="text-white/70 text-sm mt-0.5">
                        {form.data.message_type === 'important' ? 'Send an urgent popup alert' : 'Send a notification to athletes or coaches'}
                    </p>
                </div>

                <form onSubmit={submit} className="p-6 space-y-4 overflow-y-auto">

                    {/* Message Type (managers only) */}
                    {isManager && (
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1.5">Message Type</label>
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                >
                                    ...
                                    <p className="text-[10px] font-normal text-gray-400 mt-0.5">Goes to inbox · Long text OK</p>
                                </button>
                                <button
                                    type="button"
                                ...
                                >
                                    ...
                                    <p className="text-[10px] font-normal text-gray-400 mt-0.5">Popup on login · Max 200 chars</p>
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1.5">
                            {isManager ? 'Send To — Athletes' : 'Send To'}
                        </label>
                        <div className="grid grid-cols-3 gap-2">
                            <button type="button" onClick={() => selectRecipient('club')} className={btnCls(isActiveRecipient('club'))}>All Athletes</button>
                            <button type="button" onClick={() => selectRecipient('group')} className={btnCls(isActiveRecipient('group'))}>Group</button>
                            <button type="button" onClick={() => selectRecipient('user', 'athlete')} className={btnCls(isActiveRecipient('user', 'athlete'))}>Athlete</button>
                        </div>

                        {isManager && (
                            <>
                                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1.5 mt-3">Send To — Coaches</label>
                                <div className="grid grid-cols-2 gap-2">
                                    <button type="button" onClick={() => selectRecipient('coaches')} className={btnCls(isActiveRecipient('coaches'))}>All Coaches</button>
                                    <button type="button" onClick={() => selectRecipient('user', 'coach')} className={btnCls(isActiveRecipient('user', 'coach'))}>Specific Coach</button>
                                </div>
                            </>
                        )}
                    </div>
                    ...
                    {/* Athlete / Coach person dropdown */}
                    {form.data.recipient_type === 'user' && (
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1.5">
                                {userListTarget === 'coach' ? 'Select Coach' : 'Select Athlete'}
                            </label>
                            <select
                                ...
                                className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all"
                            >
                                <option value="">Choose {userListTarget === 'coach' ? 'coach' : 'athlete'}...</option>
                                {(userListTarget === 'coach' ? coaches : athletes).map(p => (
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-1.5">
                            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide">
                                {form.data.message_type === 'important' ? 'Alert Message' : 'Message'}
                            </label>
                            {form.data.message_type === 'important' && (
                                <span className={`text-xs font-bold ${bodyRemaining < 20 ? 'text-red-600' : 'text-gray-400'}`}>
                                    {bodyRemaining} / {IMPORTANT_MAX} chars left
                                </span>
                            )}
                        </div>
                        <textarea
                            ...
                            className={`w-full rounded-xl border bg-gray-50 px-4 py-3 text-sm text-gray-900 focus:bg-white focus:outline-none transition-all resize-none ${
                                form.data.message_type === 'important'
                                    ? 'border-red-200 focus:border-red-400 focus:ring-2 focus:ring-red-400/20'
                    ...
                    <div className="flex gap-3 pt-1">
                        ...
                        <button
                            ...
                            className={`flex-1 py-2.5 text-white text-sm font-bold rounded-xl transition-all disabled:opacity-50 shadow-sm ${
                                form.data.message_type === 'important'
                                    ? 'bg-red-600 hover:bg-red-700'
                                    : 'bg-indigo-600 hover:bg-indigo-700'
                            }`}
                        >
                            {form.processing ? 'Sending…' : form.data.message_type === 'important' ? 'Send Alert' : 'Send Message'}
                        </button>
                    </div>
                </form>
            </div>
...
export default function MessagesIndex(props: Props) {
    ...
    const [coachTab, setCoachTab] = useState<'inbox' | 'sent'>('inbox');
    ...
    const headerDesc = () => {
        ...
        const unread = (props as AthleteProps).messages.filter(m => !m.is_read).length;
        ...
        >
            ...
            <div className="py-8">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                        ...
                        <>
                            {props.sent.length === 0 ? (
                                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-16 text-center">
                                    ...
                                    <p className="text-sm text-gray-500 mt-1 mb-5">Send your first message to athletes or coaches.</p>
                                    <button
                                        onClick={() => setShowCompose(true)}
                                ...
                                <SentList messages={props.sent} onDelete={deleteMessage} />
                            )}
                        </>
                    )}

                    {/* ── Coach View (tabs: inbox + sent) ── */}
                    {props.role === 'coach' && (
                        <>
                            {/* Tabs */}
                            <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
                                ...
                                >
                                    Inbox
                                    {props.inbox.filter(m => !m.is_read).length > 0 && (
                                        <span className="ml-1.5 px-1.5 py-0.5 bg-red-500 text-white text-[10px] font-black rounded-full">
                                            {props.inbox.filter(m => !m.is_read).length}
                                        </span>
                                    )}
                                </button>
                                <button
                            ...
                            {coachTab === 'inbox' && <InboxList messages={props.inbox} />}

                            {coachTab === 'sent' && (
                                props.sent.length === 0 ? (
                                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-12 text-center">
                                        <p className="font-bold text-gray-900 text-sm">No messages sent yet</p>
                                        <p className="text-xs text-gray-500 mt-1 mb-4">Send a message to your athletes.</p>
                                        <button onClick={() => setShowCompose(true)} className="px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition-all">
                                            Compose
                                        </button>
                                    </div>
                                ) : (
                    ...
                    {/* ── Athlete/Parent Inbox ── */}
                    {props.role === 'athlete' && (
                        <InboxList messages={(props as AthleteProps).messages} />
                    )}
            ...
            {showCompose && canCompose && (
                <ComposeModal
                    groups={(props as ManagerProps | CoachProps).groups}
                    athletes={(props as ManagerProps | CoachProps).athletes}
                    coaches={props.role === 'manager' ? (props as ManagerProps).coaches : []}
